...
import 'package:quacker/client/nitter_backend.dart'; // ✅ استيراد Nitter Backend
...
class _QuackerTwitterClient extends TwitterClient {
  ...
  @override
  Future get(Uri uri, {Map? headers, Duration? timeout}) {
  ...
  static Future fetch(Uri uri, {Map? headers}) async {
    final XRegularAccount model = XRegularAccount();
    final authHeader = await TwitterHeaders.getAuthHeader();
    if (authHeader != null) {
      return await model.fetch(uri,
          headers: headers, log: log, authHeader: authHeader);
    } else {
...
class UnknownProfileResultType
...
class UnknownProfileUnavailableReason
    with SyntheticException
    implements Exception {
  final String reason;
  final String uri;
...
class Twitter {
  static final TwitterApi _twitterApi =
      TwitterApi(client: _QuackerTwitterClient());
  static final FFCache _cache = FFCache();

  // ✅ التحكم في استخدام Nitter بدلاً من Twitter
  static const bool useNitter = true; // غيّرها إلى false لاستخدام Twitter الأصلي
  static final NitterBackend _nitter =
      NitterBackend(host: 'nitter.net');
  static final Logger _log = Logger('Twitter');

  static Map defaultParams = {
  ...
  static Future getProfileById(String id) async {
    var uri = Uri.https(
      'twitter.com',
      '/i/api/graphql/Qs44y3K0SXxItjNi6mUFQA/UserByRestId',
      {
  ...
  // ✅ دالة معدّلة لاستخدام Nitter
  static Future<Profile> getProfileByScreenName(String screenName) async {
    // أولاً: جرّب Nitter إذا useNitter = true
    if (useNitter) {
      _log.info('🔵 [Nitter] Loading profile for @$screenName');
      try {
        return await _nitter.getProfileByScreenName(screenName);
      } catch (e, st) {
        _log.severe(
            '❌ Nitter profile failed for @$screenName: $e\n$st');
        rethrow; // أو يمكنك هنا fallback للـ Twitter لو حاب
      }
    }

    // 🟦 السلوك الأصلي (Twitter GraphQL) في حال أردت استخدامه
    _log.info('🟦 [Twitter] Loading profile for @$screenName');

    var uri = Uri.https(
      'twitter.com',
      '/i/api/graphql/qW5u-DAuXpMEG0zA1F7UGQ/UserByScreenName',
      {
        'variables': jsonEncode({
  ...
  static Future _getProfile(Uri uri) async {
    ...
    if (hasErrors && content['errors'] != null) {
      ...
      if (errors.isEmpty) {
        throw TwitterError(
            code: 0, message: 'Unknown error', uri: uri.toString());
      } else {
        throw TwitterError(
    ...
    if (resultType != null) {
      switch (resultType) {
        case 'UserUnavailable':
          ...
          if (code == 'Suspended') {
            throw TwitterError(
                code: 63, message: result['reason'], uri: uri.toString());
          } else {
            throw TwitterError(
                code: -1, message: result['reason'], uri: uri.toString());
          }
    ...
    var pins = List.from(result['legacy']['pinned_tweet_ids_str']);
    return Profile(user, pins);
  }

  static Future getProfileFollows(String screenName, String type,
      {int? cursor, int? count = 200}) async {
    var response = type == 'following'
        ? await _twitterApi.userService.friendsList(
            screenName: screenName, cursor: cursor, count: count, skipStatus: true)
  ...
  static bool isNotPromoted(Map item) {
  ...
  static List createTweetChains(List addEntries) {
    List replies = [];
    for (var entry in addEntries) {
      ...
      if (entryId.startsWith('tweet-')) {
        ...
        if (tweetResults['result']["__typename"] ==
  ...
  static List createTweets(List addEntries, [bool isPinned = false]) {
    List replies = [];
    for (var entry in addEntries) {
      var entryId = entry['entryId'] as String;
      if (entryId.startsWith('tweet-')) {
          ...
          if (itemType == 'TimelineTweet') {
            if (item['item']['itemContent']['tweet_results']?['result'] !=
              ...
              if (item['item']['itemContent']['tweet_results']['result']
                  ['tweet'] ==
                  null) {
                var tweet = TweetWithCard.fromGraphqlJson(item['item']
  ...
  static Future getTweet(String id, {String? cursor}) async {
    Map defaultParam = {
      "variables":
    ...
    Map variables = json.decode(defaultParam["variables"].toString());
    ...
    var response = await _twitterApi.client.get(Uri.https(
        'twitter.com', '/i/api/graphql/3XDB26fBve-MmjHaWTUZxA/TweetDetail', defaultParam));
    var result = json.decode(response.body);
    var instructions = List.from(result?['data']?['threaded_conversation_with_injections_v2']
            ?['instructions'] ??
        []);
    ...
    var addEntriesInstructions =
        instructions.firstWhereOrNull((e) => e['type'] == 'TimelineAddEntries');
    ...
    var repEntries =
        List.from(instructions.where((e) => e['type'] == 'TimelineReplaceEntry'));
  ...
  static Future searchTweets(String query, bool includeReplies,
  ...
  static Future<List<UserWithExtra>> searchUsers(String query,
    ...
    List instructions = List.from(result?['data']?['search_by_raw_query']
    ...
    List addEntries = List.from(instructions
            .firstWhere((e) => e['type'] == 'TimelineAddEntries', orElse: () => null)
            ?['entries'] ??
        []);
  ...
  static Future<List<TrendLocation>> getTrendLocations() async {
    var result = await _cache.getOrCreateAsJSON(
        'trends.locations', const Duration(days: 2), () async {
  ...
  static Future<List<Trends>> getTrends(int location) async {
    var result =
        await _cache.getOrCreateAsJSON('trends.$location', const Duration(minutes: 2), () async {
  ...
  static Future getTimelineTweets(
    String id,
    String type, {
    List? pinnedTweets,
    int count = 10,
    ...
    Map variables = json.decode(defaultUserTweetsParam["variables"].toString());
    variables["userId"] = id;
    ...
    var response = await _twitterApi.client.get(Uri.https('twitter.com',
        'i/api/graphql/W4Tpu1uueTGK53paUgxF0Q/HomeTimeline', defaultUserTweetsParam));
    var result = json.decode(response.body);
  ...
  static Future getTweets(String id, String type, List pinnedTweets,
      {int count = 10,
      String? cursor,
      bool includeReplies = true,
      bool includeRetweets = true,
    ...
    var query = {
      ...
      'include_want_retweets':
          includeRetweets ? '1' : '0', // This may not actually do anything
    ...
    defaultUserTweetsParam["variables"] = json.encode(variables);
    var response = await _twitterApi.client.get(Uri.https('twitter.com',
        '/i/api/graphql/2GIWTr7XwadIixZDtyXd4A/UserTweets', defaultUserTweetsParam));
    ...
    return createUnconversationedChains(
        result, 'tweet', pinnedTweets, includeReplies == false, includeReplies, showPinnedTweet,
        getTweetsCounter, incrementTweetsCounter);
  }

  static String? getCursor(List addEntries, List repEntries, String legacyType,
      String type) {
    String? cursor;
    Map? cursorEntry;
    var isLegacyCursor = addEntries.any((element) => element['entryId'].startsWith('cursor'));
    if (isLegacyCursor) {
      cursorEntry = addEntries.firstWhere((e) => e['entryId'].contains(legacyType),
          orElse: () => null);
    } else {
      cursorEntry = addEntries
          .where((e) => e['entryId'].startsWith('sq-C'))
          .firstWhere((e) => e['content']['operation']['cursor']['cursorType'] == type,
              orElse: () => null);
    }
  ...
  static TweetStatus createUnconversationedChainsGraphql(Map result, String tweetIndicator,
      List pinnedTweets, bool mapToThreads, bool includeReplies) {
    var instructions = List.from(result['timeline']['instructions']);
    if (instructions.isEmpty ||
        !instructions.any((e) => e['type'] == 'TimelineAddEntries')) {
      return TweetStatus(chains: [], cursorBottom: null, cursorTop: null);
    }

    var addEntries =
        List.from(instructions.firstWhere((e) => e['type'] == 'TimelineAddEntries')['entries']);
    var repEntries =
        List.from(instructions.where((e) => e['type'] == 'TimelineReplaceEntry'));
    ...
    for (var conversation
        in conversations.entries.sorted((a, b) => b.key.compareTo(a.key))) {
      var chainTweets =
          conversation.value.sorted((a, b) => a.idStr!.compareTo(b.idStr!)).toList();
  ...
  static TweetStatus createUnconversationedChains(
    ...
    var instructions =
        List.from(result["data"]["user"]["result"]["timeline_v2"]['timeline']['instructions']);
    var addEntriesInstructions =
        instructions.firstWhereOrNull((e) => e['type'] == 'TimelineAddEntries');
    ...
    var addPinnedTweetsInstructions =
        instructions.firstWhereOrNull((e) => e['type'] == 'TimelinePinEntry');
    var addEntries = List.from(addEntriesInstructions['entries']);
    var repEntries =
        List.from(instructions.where((e) => e['type'] == 'TimelineReplaceEntry'));

    List addPinnedEntries = List.empty(growable: true);
  ...
  static Future<List<UserWithExtra>> getUsers(Iterable ids) async {
  ...
  static Future<List<UserWithExtra>> _getUsersPage(Iterable ids) async {
    var response = await _twitterApi.client.get(Uri.https('api.twitter.com',
  ...
  static Map<String, TweetWithCard> _createTweetsGraphql(
      String entryPrefix, List allTweets, bool includeReplies) {
  ...
  static Future<dynamic> getBroadcastDetails(String key) async {
    var response = await _twitterApi.client
        .get(Uri.https('twitter.com', '/i/api/1.1/live_video_stream/status/$key'));
...
class TweetWithCard extends Tweet {
  ...
  factory TweetWithCard.tombstone(dynamic e) {
    ...
    tweetWithCard.text =
        ((e['richText']?['text'] ?? e['text']?['text'] ?? L10n.current.this_tweet_is_unavailable)
                as String)
  ...
  factory TweetWithCard.fromGraphqlJson(Map<String, dynamic> result) {
    ...
    if (result['tweet'] != null) {
      ...
      retweetedStatus =
          TweetWithCard.fromGraphqlJson(result['legacy']['retweeted_status_result']['result']!);
    }
  ...
  factory TweetWithCard.fromCardJson(
      Map<String, dynamic> tweets, Map<String, dynamic> users, Map<String, dynamic> e) {
    var user =
        e['user_id_str'] == null ? null : UserWithExtra.fromJson(users[e['user_id_str']]);
  ...
  factory TweetWithCard.fromData(Map<String, dynamic> e, String? noteText, Entities? noteEntities,
      UserWithExtra? user, TweetWithCard? retweetedStatus, TweetWithCard? quotedStatus) {
    ...
    tweet.extendedEntities =
        e['extended_entities'] == null ? null : Entities.fromJson(e['extended_entities']);
    ...
    tweet.displayTextRange =
        (e['display_text_range'] as List?)?.map((e) => e as int).toList();
...
class TweetChain {
  ...
  factory TweetChain.fromJson(Map<String, dynamic> e) {
    var tweets =
        List.from(e['tweets']).map((e) => TweetWithCard.fromJson(e)).toList();
...
class TweetStatus {
  ...
  TweetStatus(
      {required this.chains, required this.cursorBottom, required this.cursorTop});
}
